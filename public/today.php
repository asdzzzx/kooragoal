...
if ($needsRefresh) {
    $updater = new UpdateManager($db, $container->get(ApiClient::class), $container->get(Logger::class));
    $updater->updateDailyFixtures(new DateTimeImmutable('now'));
    $fixtures = $db->fetchAll(
        'SELECT f.*, th.name AS home_name, ta.name AS away_name, l.name AS league_name
         FROM fixtures f
         JOIN teams th ON th.id = f.home_team_id
        ...
        <div class="col-md-6">
            <div class="card shadow-sm h-100">
                <div class="card-body">
                    ...
                    <a href="/match/<?= (int) $fixture['id'] ?>" class="btn btn-outline-primary btn-sm mt-3">التفاصيل</a>
                </div>
            </div>
        </div>
    <?php endforeach; ?>
