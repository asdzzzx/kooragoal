...
## قالب ووردبريس جاهز للرفع
- داخل المجلد الجذري للمشروع ستجد مجلد القالب `kooragoal-theme` مباشرة، ويحتوي على جميع الملفات المطلوبة (ومن ضمنها `style.css` في الجذر كما تشترطه ووردبريس).
- قم بضغط المجلد `kooragoal-theme` بصيغة ZIP (أو استخدم السكربت `./scripts/package-theme.sh` الموضح أدناه) ثم ارفعه عبر لوحة تحكم ووردبريس (`المظهر > أضف جديد > رفع قالب`).
- من صفحة إعدادات القالب (`المظهر > إعدادات Kooragoal`) حدد رابط ملف الـAPI، البطولات الافتراضية، وصفحة تفاصيل المباراة التي تستخدم قالب **Kooragoal Match Center**.
- أضف الصور المطلوبة داخل `assets/images` بالقالب (`logo.svg`, `match-card-bg.png`, `lineup-pitch.png`, وأيقونات البطاقات والأهداف) قبل التفعيل للحصول على المظهر النهائي.

### إنشاء ملف ZIP جاهز للرفع
يتوفر سكربت مبسط ضمن `scripts/package-theme.sh` يقوم بإنشاء ملف `kooragoal-theme.zip` في مجلد `dist/` يحتوي على القالب فقط، ويمكن رفعه مباشرة إلى ووردبريس:
...
سيتم إنشاء مجلد `dist` (إن لم يكن موجودًا) ووضع الملف المضغوط داخله، مع التحقق من وجود ملف `style.css` في جذر القالب قبل البدء.
